...
public class DanceKinectManager : MonoBehaviour 
{
...
//	public bool startKinectServer = true;
	...
	private float MapsPercentWidth = 1f;
	private float MapsPercentHeight = 1f;
    ...
    private static DanceKinectManager instance = null;
...
//	private int maxDepth;
    ...
    public static DanceKinectManager Instance
    {
        get
        {
            return instance;
...
//			}
	...
	public void setLiveStream(bool status){
			displayColorMap = status;
	}

	void updateSkeletonIndex(){
		Debug.Log (GetUserPosition(GetUserIdByIndex(0)));
	}
		...
		if(kinectInitialized)
		{
			...
			foreach (AvatarController controller in avatarControllers)
			{
				...
				if((userIndex >= 0) && (userIndex < alUserIds.Count))
				{
					...
					//Int64 userId =GetUserId_(userIndex);
...
//			UpdateRegions();
	...
	void UpdateColorMap()
	{
		...
		usersClrTex.Apply();
		liveFeed.GetComponent<Renderer>().material.SetTexture("_MainTex",usersClrTex);
	}
			...
			if(bodyData.bIsTracked != 0 && Mathf.Abs(kinectToWorld.MultiplyPoint3x4(bodyData.position).z) >= minUserDistance)
			{
				...
				for (int j = 0; j < jointsCount; j++)
				{
						...
						bodyData.joint[j].trackingState != TrackingState.NotTracked)
					{
						...
						{
							calibrationText.GetComponent<GUIText>().text = debugText;
						}
					}
				}
...
//							jointData.mirroredRotation = Quaternion.Euler(mirroredAngles);
	...
	public Int64 GetUserId_(int i){
		return alUserIds[i];
	}

	public int GetUserIndex_(Int64 liUserId){
		return dictUserIdToIndex[liUserId];
	}	
